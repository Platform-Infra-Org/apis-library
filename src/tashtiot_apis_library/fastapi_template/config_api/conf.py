...
Unlike the generic, now client-side SSO ([`SSOConfig`][SSOConfig]), these are read from
...
class ConfigRemoteSettings(BaseSettings):
    ...
    * ``"sso"`` -- OAuth2 ``client_credentials`` bearer minted from the
      ``CONFIG_REMOTE_SSO_*`` knobs (client-side [`SSOConfig`][SSOConfig]).
    ...
    * ``"none"`` -- no auth (plain HTTP; e.g. in-cluster / mesh-secured / local dev).
    ...
    def sso_config(self) -> SSOConfig:
        """Build the client-side [`SSOConfig`][SSOConfig] from the ``CONFIG_REMOTE_SSO_*`` knobs."""
        return SSOConfig(
    ...
    def resolve_auth(self) -> Tuple[Optional[httpx.Auth], Dict[str, Any]]:
        ...
        flow into the provider's outbound client. Raises [`AuthConfigError`][AuthConfigError]
